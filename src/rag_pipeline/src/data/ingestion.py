...
import requests
import numpy as np
# Prefer the modern PyMuPDF import name; fall back to legacy 'fitz' if needed
try:
    import pymupdf  # PyMuPDF >= 1.24
except ImportError:  # pragma: no cover - runtime env compatibility
    try:
        import fitz as pymupdf  # PyMuPDF < 1.24 exposes 'fitz'
    except ImportError as _e:
        # Re-raise with a clearer hint for users
        raise ImportError("PyMuPDF is required. Install with 'pip install pymupdf' (or conda-forge).") from _e

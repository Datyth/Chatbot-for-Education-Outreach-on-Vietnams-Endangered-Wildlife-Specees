...
# Ensure project root (which contains `rag_pipeline/`) is on sys.path
CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
PROJECT_ROOT = os.path.dirname(CURRENT_DIR)
if PROJECT_ROOT not in sys.path:
    sys.path.insert(0, PROJECT_ROOT)

# Load the local rag_pipeline module by file path to avoid
# conflicts with any installed packages named `rag_pipeline`.
RAG_PIPELINE_PATH = os.path.join(PROJECT_ROOT, "rag_pipeline", "rag_pipeline.py")
